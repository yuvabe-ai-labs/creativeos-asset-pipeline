...
> *Review copy for comments — the live version is at https://claude.ai/code/artifact/d0b34f4a-dba5-4d7d-9a87-ce29bbefb781*
...
> write-up: [Shopify integration research](https://claude.ai/code/artifact/5e46e0cb-e26b-4eeb-a195-f1034bd009b9).
...
| **Evals for continuous improvement** | **We need to see more data and improve our prompts with it.** Today nothing measures what the models produce. Evals make looking routine — real outputs reviewed, patterns spotted, prompts improved with evidence instead of guessing, all on actual client work. |
...
the [Market Signals PRD](https://claude.ai/code/artifact/4d7dfa84-0ece-4cc8-a0df-d474062d453c).
...
| **Evals** | Measurement on real client work: outputs labelled, signals compared on/off, sameness tracked. | Automatic grouping of failures; AI scoring; learning from corrections. |
...
So **sprint 1 is signals** — measured on real client work, because that's how we check signals
...
> one usable slice**: MR enters a signal, a designer applies it to a script or prompt, the
> output changes, and we can measure it. In-canvas browsing, the rest of the chain and the
> review queue come in sprint 3 and phase 2. Whether even the slice fully fits is flagged
> as an open question in the
> [Market Signals PRD](https://claude.ai/code/artifact/4d7dfa84-0ece-4cc8-a0df-d474062d453c) (§12), with a
...
> **[Shopify integration research](https://claude.ai/code/artifact/5e46e0cb-e26b-4eeb-a195-f1034bd009b9)**.

### 6.2 Signals — how they reach the work

| | | |
|---|---|---|
| R2.1 | Signals are available through the client automatically; a signal influences a generation only when someone deliberately applies it to that piece of work. | V2 |
| R2.2 | How signals surface in the canvas — controls, placement, any on-canvas presence — is **decided in the signals design spec after the PRD is discussed**, not here. | V2 |
| R2.3 | However a signal reaches a generation, one piece of code resolves it — same signal, same result, everywhere. | V2 |
...
| | | |
|---|---|---|
| R6.1 | Measurement runs on **real client work**, through the eval screen we already have — no synthetic test sets. | V2 |
| R6.2 | **Someone reads real outputs and writes notes**, then those notes get grouped into a ranked list of what's going wrong. A person reads first; AI can help group them after the first 30–50 are done by hand. | V2 |
| R6.3 | **Past changes that shipped unmeasured** — like the shot-context trim — get checked against real outputs too. | V2 |
...
| R6.5 | Compare **signals on versus signals off** on the same real scripts. | V2 |
...
| **Sameness is measured, and moving the right way** | The first time in this product's life we'd have proof a change did what it was meant to. |
...
1. ~~Whose job is it to add signals?~~ **Answered: the MR team** — see §3 and the
   [Market Signals PRD](https://claude.ai/code/artifact/4d7dfa84-0ece-4cc8-a0df-d474062d453c). The remaining signal
...
| Doc | Covers | Status |
|---|---|---|
| [Market Signals PRD](https://claude.ai/code/artifact/4d7dfa84-0ece-4cc8-a0df-d474062d453c) | Market signals **PRD** — the record, scopes, the MR team's process, both consumption modes, the manual→agent path, sprint mapping. Its design spec follows once MR discovery closes | **PRD drafted — in review** |
...
| [Shopify research](https://claude.ai/code/artifact/5e46e0cb-e26b-4eeb-a195-f1034bd009b9) | Background reading — whether Shopify is worth integrating, and where "past performance" signals could come from | Research, no decision taken |
