...
- **Spec:** `docs/superpowers/specs/2026-07-28-video-start-end-spine-design.md` (decisions D81–D86).
...
The O1 config was built from third-party wrapper docs (fal.ai / WaveSpeed), whose limits are narrower than Kling's. This task fixes the settings and splits the shared capability descriptor (D85).
...
```ts
// D85: 3.0 and O1 have different reference mechanisms (registered `element` vs inline
...
and the shared capability descriptor splits per model (D85). Also fixes
...
— the 7-image omni budget less both frames (D86).
...
## Task 4: Write locked params into state (D84)
...
```ts
/**
 * D84 — locked parameter values are the source of truth, not just a display substitution.
 *
...
```ts
  // D84: the panel displays locked values, but `params` is what doGenerate posts. Without this,
...
for 11 observed generation failures after constraint rules shipped (D84).
...
## Task 5: Reject rule violations server-side (D83)

The route never evaluates rules, so any path that bypasses the focus view sends illegal
combinations straight to the vendor. Per D83 the server **rejects** and never auto-corrects.
...
```ts
/**
 * D83 — the server rejects, it never corrects. Returns the reason a request violates the
...
```ts
  // D83: reject rather than correct. The UI evaluates the same rules and should never let an
...
reason and never mutates params (D83).
...
/**
 * D81 — the opinion is expressed by layout. This model never carries a blocking state; a
...
generate. The blocking missing-end-frame AlertDialog is removed: per D81
...
## Task 7: Derive the end frame from the start frame (D82)
...
/**
 * D82 — the end frame is an EDIT of the start frame, not a fresh generation. Interpolation
...
| §5.3 UI reconciliation (D84) | 4 |
| §5.3 new Kling rules | 3 |
| §5.3 server rejects (D83) | 5 |
| §5.4 Kling config corrections (D85) | 1 |
| §5.5 Kling O1 references (D86) | 2, 3 |
