...
// POST /api/clients/:id/kb/extract
// Runs document extraction + image analysis in parallel, merges results,
// stores as a new KB version, sets it active, and sets kb_status = 'in_review'.
export async function POST(
  ...
  return withClient(params, async (clientId) => {
    return withTryCatch("Extraction failed", async () => {
      ...
      if (docs.length === 0) {
        return apiError("Upload at least one document before extracting.", 400);
      }
