// The one snapshot path both callers use — the daily trigger sweep
// (trigger/snapshot-handles.ts) and the refresh route, which the client also fires for a
// just-added handle's first fetch (D275) — mirroring ingestReference's contract: the
// SNAPSHOT always saves; thumbnails are best-effort decoration (D185's spirit). Only a
// DB/provider failure propagates.
//
