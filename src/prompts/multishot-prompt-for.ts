...
import { KLING_OMNI_MODEL_ID, SEEDANCE_MODEL_ID } from "@/lib/video-gen/client-models";
import {
 ...
 * limits and another model's prompt.
 */
export function multishotPromptFor(
  targetModel: string | undefined | null,
): MultishotPromptSpec {
  const cap = multishotCapabilityFor(targetModel);
  if (cap.id === KLING_OMNI_MODEL_ID) return multishotPromptKling();
  if (cap.id === SEEDANCE_MODEL_ID) return multishotPromptSeedance();
  return multishotPromptGenerate();
}
